...
- **Predicting signal quality** (RSSI, SNR, path loss) using ensemble ML models  
- **Calculating packet delivery rate (PDR)** using LoRaWAN physics (not ML-predicted)  
- **Finding optimal beacon placement** using A* pathfinding algorithm  
- **Fetching real terrain data** from Google Earth Engine (elevation, land cover)  
- **Visualizing results** with interactive HTML maps  
...
- **Disk caching**: Reuses previously fetched data (82,481 points cached as of Oct 19, 2025)  

### 3. Machine Learning Models
- **Random Forest**: Fast, interpretable  
- **XGBoost**: High accuracy, gradient boosting (**selected as best model**)  
...
- **Land-cover-specific recovery constants**:  
  - Water (`k = 0.40`): Fast PDR recovery  
  - Cropland (`k = 0.28`): Moderate recovery  
  - Buildings (`k = 0.08`): Slow recovery due to multipath  
...
│  • Creates search corridor (±3-6 km width)                  │
...
│  • Uses BEST model (XGBoost selected in Oct 19 run)         │
...
│  Formula: PDR = 1 / (1 + exp(-k × (margin - 5)))            │
...
```bash
git clone https://github.com/Nfx1z/Predictive_Optimization_LoRa
cd Predictive_Optimization_LoRa
```
...
| direct_path_use_grid | bool | True/False | True | Match grid or use custom distance |
| direct_path_samples | int | 1-100 | NaN | Fixed number of samples (if use_grid=False) |
| direct_path_spacing_km | float | 0.1-10.0 | NaN | Custom spacing (if use_grid=False) |
...
- `path_avg_penalty, path_elevation_std, max_terrain_obstruction_m, path_dominant_land_cover`
...
# - Neural Network (100 trials with Optuna)
# - Ensemble (weighted combination)

# 3. Best model auto-selected based on R²
print(f"Best model: {model_name}")  # e.g., "XGBoost"
...
### Training Performance (October 19, 2025 Run)
| Model | RSSI R² | SNR R² | Path Loss R² | Avg R² |
|-------|---------|--------|--------------|--------|
| XGBoost | **0.8104** | **0.5229** | **0.8104** | **0.7146** |
| Random Forest | 0.8135 | 0.4998 | 0.8135 | 0.7089 |
| Neural Network | 0.6721 | 0.3456 | 0.6749 | 0.5642 |
| Ensemble | 0.8048 | 0.5052 | 0.8051 | 0.7050 |

> **Best Model**: XGBoost (Average R² = 0.7146)

### Training Time
- **Random Forest**: ~3 minutes (500 iterations)  
- **XGBoost**: ~3 minutes (500 iterations)  
- **Neural Network**: ~25 minutes (100 Optuna trials)  
- **Total**: ~31 minutes  
...
def calculate_lora_cost(self, pdr, terrain_penalty, distance, land_cover):
    ...
    # Total cost (70% PDR, 10% distance, 20% terrain)
    return pdr_cost * 0.7 + distance_cost * 0.1 + terrain_cost * 0.2
```

### PDR Physics Formula
```python
def calculate_pdr(self, snr, spreading_factor, land_cover):
    snr_threshold = self.snr_thresholds.get(spreading_factor, -7.5)
    margin = snr - snr_threshold
    
    # CRITICAL: Below threshold = near-zero PDR
    if margin <= -5:
    ...
    # Sigmoid-like recovery (more realistic)
...
# Example from Oct 19 run:
# Neural_Network: weight = 0.193
# Random_Forest:  weight = 0.398  
# XGBoost:        weight = 0.409
```
...
**Error**: `InvalidCoordinatesError: Start latitude 150.841 out of range [-90, 90]. Did you swap latitude and longitude?`  
**Solution**: Check coordinate order (*lat, lon* not lon, lat)

#### 3. No Viable Path Found
**Error**: `NoViablePathError: No viable path found with given parameters.`  
**Solutions**:
...
**Solutions**:
- Increase workers: `gee_workers=8` (faster)  
- Enable caching: `cache_enabled=True`  
- Reduce grid density: `grid_spacing_km=2.0`  
- Check internet connection  

**Problem**: A* takes too long (>5 minutes)  
**Solutions**:
- Increase grid spacing: `grid_spacing_km=2.0`  
- Narrow corridor: `corridor_width_km=3.0`  
- Stricter PDR: `min_pdr_threshold=0.4` (blocks more points)  
- Tighter deviation: `max_path_deviation=0.3`  
...
A: With good training data (Oct 19, 2025 run):  
- RSSI: R² = 0.8104 (±7.39 dBm)  
- SNR: R² = 0.5229 (±5.27 dB)  
...
A: Yes! Provide CSV with 15 features + 3 targets (RSSI, SNR, path_loss). Minimum 1000 samples recommended.  
...
- Increase `spreading_factor` → longer range  
- Increase `tx_power` → stronger signal  
- Widen corridor: `corridor_width_km=6.0`  
